...
use crate::{
    address_book::{
        ...
        UniV2RouterCalls,
        UNISWAPV3_FACTORY,
        SUSHIV2_FACTORY_ARBITRUM,
        SUSHISWAPV2_FACTORY,
        UNISWAPV2_FACTORY,
        VVS_FACTORY,
        PANCAKESWAP_FACTORY,
        TRADERJOE_FACTORY_ARBITRUM,
    },
    timestamp_print
};
...
impl Dex {
    ...
    */
    ...
    pub async fn token_stream_pairs_created(
        ...
        println!("----------------------- {} | {} -----------------------", self.name, self.chain);
        timestamp_print!(
            Color::BrightBlue, Some(false), 
            format!("[{}] Listening for PairCreated events, on token {} from {}", 
                token_address, 
                self.name, 
        ...
        while let Some(Ok((event, meta))) = stream.next().await {
            if event.token_0 == token_address || event.token_1 == token_address {
                timestamp_print!(Color::Green, Some(false), format!("[PAIR][{}] PairCreated detected: {:#066x}", dex.name, event.pair));
                let pair = Pair::new(
                    event.pair,
                    event.token_0,
                    event.token_1,
                ...
                tokio::spawn(async move {
                    pair.stream_add_liquidity(ws_clone, http_clone, meta.block_number).await.unwrap();
                });
                timestamp_print!(Color::BrightBlue, Some(false), format!("[PAIR][{}] Listening for new PairCreated events, from {}", dex.name, dex.factory_address));
            }
        }
        Ok(())  
    }
    ...
    pub async fn stream_pairs_created(
        ...
        println!("----------------------- {} | {} -----------------------", self.name, self.chain);
        timestamp_print!(Color::BrightBlue, Some(false), format!("[{}] Listening for PairCreated events, from {}", self.name, self.factory_address));
        let contract = UniV2Factory::new(self.factory_address, ws.clone());
        let events = contract.events();
        let mut stream = events.stream().await?.with_meta();
